...
登录注册模块使用飞书多维表格存储用户，配置步骤见 [`docs/飞书登录注册配置.md`](./docs/飞书登录注册配置.md)。未配置飞书凭证时，公开页面可访问，登录/注册接口会返回配置提示。

数字员工、能力、合同、任务、结算、评价和审计等产品数据的多维表格模型见 [`docs/飞书产品数据模型.md`](./docs/飞书产品数据模型.md)。
